...
import (
	...
	"github.com/Max-Gabriel-Susman/bestir-matchmaking-service/internal/handler"
...
func run(ctx context.Context, _ []string) error {
	...
	var cfg struct {
		ServiceName string `env:"SERVICE_NAME" envDefault:"bestir-matchmaking-service"`
		Env         string `env:"ENV" envDefault:"local"`
		Database    struct {
			User   string `env:"MATCH_DB_USER,required"`
			Pass   string `env:"MATCH_DB_PASSWORD,required"`
			Host   string `env:"MATCH_DB_HOST"`
			Port   string `env:"MATCH_DB_PORT" envDefault:"3306"`
			DBName string `env:"MATCH_DB_Name" envDefault:"match"`
			Params string `env:"MATCH_DB_Param_Overrides" envDefault:"parseTime=true"`
		}
		Datadog struct {
	...
	*/
	...
	// 	return err
	// }
	// log.Println("successfuly created match table")
	//*/
